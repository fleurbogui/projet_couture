...
<html>
<head>
    <title>Despo Couture - Suppression de commande</title>
...
<body class="d-flex flex-column min-vh-100">
    ...
    <div class="container p-4">
        <?php
            ...
            showMessage();
        ?>
        <h2>Suppression de la commande</h2>
            ...
            <?php
                ...
                if($_SERVER["REQUEST_METHOD"] == 'GET') {
                    if (!isset($_GET['ref']) || !is_numeric($_GET['ref'])) {
                        sendMessage("Référence de commande incorrecte", "error", "formDeleteCommande.php");
                        exit();
                    }
                    $commande = getCommandeById($_GET['ref']);

                    if ($commande === null) {
                        sendMessage("Commande introuvable", "error", "formDeleteCommande.php");
                        exit();
                    }
                    ...
                    else
                    {
                        ...
                        echo "<label for='confirmation'>Confirmez-vous la suppression de la commande de référence " .$commande["RefCom"]. " ?</label>";
                        echo "</div>";
                        echo "<button type='submit' class='btn btn-danger mr-3'>Supprimer</button>";
                        echo "<a href='gestionCommandes.php' class='btn btn-secondary'>Annuler</a>";
                    }
                }
            ?>
    </div>
    ...
    <?php
        if($_SERVER["REQUEST_METHOD"] == 'POST')
        {
            ...
            $commande = getCommandeById($_POST['id']);
            ...
            $result = deleteCommande($_POST['id']);

            if($result)
            {
                sendMessage("Commande supprimée avec succès", "success", "gestionCommandes.php");
            }
            else
            {
                sendMessage("Erreur lors de la suppression de la commande", "error", "gestionCommandes.php");
            }
        }
    ?>
